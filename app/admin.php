...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if($act==='site_settings'){
        ...
        set_site_setting('point_name', clean($_POST['point_name']??'积分',20));
        $msg='站点设置已保存';
        $pn=point_name();
...
foreach($users as $x) row_item('admin.php',$x['avatar_text'],$x['username'],$x['role'].' · '.$x['points'].' '.$pn);
shell_mid();
?>
<h1><?=h(t('admin'))?></h1>
...
<div class="card"><b><?=h($x['username'])?></b><p class="muted">ID <?=h($x['id'])?> · <?=h($x['role'])?> · <?=h($x['points'])?> <?=h($pn)?> · <?=$x['is_banned']?'已封禁':'正常'?></p>
...
<form method="post" action="admin.php" style="display:inline"><input type="hidden" name="act" value="unban"><input type="hidden" name="id" value="<?=h($x['id'])?>"><button class="btn ghost">解封</button></form>
<form method="post" action="admin.php" style="display:inline"><input type="hidden" name="act" value="reset_waf"><input type="hidden" name="id" value="<?=h($x['id'])?>"><button class="btn ghost">重置 WAF</button></form><?php endif;?>
</div>
<?php endforeach;?>
