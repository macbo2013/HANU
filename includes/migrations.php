...
function hanu_run_builtin_migrations(): array {
    ...
    if (!hanu_migration_done($migration)) {
        ...
        if (hanu_table_exists($settings)) {
            q_exec("INSERT INTO " . table_name('settings') . "(name,value) VALUES(?,?) ON DUPLICATE KEY UPDATE value=VALUES(value)", ['app_version', '1.0.3']);
    ...
    if (!hanu_migration_done($migration)) {
        ...
        if (hanu_table_exists($settings)) {
            q_exec("INSERT INTO " . table_name('settings') . "(name,value) VALUES(?,?) ON DUPLICATE KEY UPDATE value=VALUES(value)", ['app_version', '1.0.3']);
            q_exec("INSERT INTO " . table_name('settings') . "(name,value) VALUES(?,?) ON DUPLICATE KEY UPDATE value=VALUES(value)", ['version_label', 'HANU V11 正式版']);
            q_exec("INSERT INTO " . table_name('settings') . "(name,value) VALUES(?,?) ON DUPLICATE KEY UPDATE value=VALUES(value)", ['site_announcement', '欢迎使用 HANU V11 正式版，感谢参与公测。']);
        }
        hanu_mark_migration_done($migration);
        $ran[] = $migration;
    }
    ...
    $migration = '2026_06_07_000002_v11_web_update_and_status';
    if (!hanu_migration_done($migration)) {
        $settings = hanu_table_raw('settings');
        if (hanu_table_exists($settings)) {
            q_exec("INSERT INTO " . table_name('settings') . "(name,value) VALUES(?,?) ON DUPLICATE KEY UPDATE value=VALUES(value)", ['app_version', '1.0.3']);
            q_exec("INSERT INTO " . table_name('settings') . "(name,value) VALUES(?,?) ON DUPLICATE KEY UPDATE value=VALUES(value)", ['version_label', 'HANU V11 正式版']);
            q_exec("INSERT INTO " . table_name('settings') . "(name,value) VALUES(?,?) ON DUPLICATE KEY UPDATE value=VALUES(value)", ['web_update_enabled', '1']);
        }

        $p = cfg('table_prefix', 'hanu_');

        db()->exec("CREATE TABLE IF NOT EXISTS `{$p}user_statuses` (
          `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
          `user_id` INT UNSIGNED NOT NULL,
          ...
          KEY `idx_visibility_time` (`visibility`,`updated_at`)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
