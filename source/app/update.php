...
<div class="grid">
  ...
  <div class="card">
    ...
    <?php else: ?>
      <p class="muted"><?=h($check['error'])?></p>
    <?php endif; ?>
  </div>
</div>

<?php if($check['ok'] && $check['has_update']): ?>
<div class="card">
  <h2>发现新版本</h2>
  <p class="muted">请先备份网站和数据库，再在服务器运行 update.sh。更新会保留 config、data、ICO，不会清空用户数据。</p>
...
<?php if($check['ok'] && !empty($check['notes'])): ?>
<div class="card">
  <h2>版本说明</h2>
