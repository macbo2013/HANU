...
if($_SERVER['REQUEST_METHOD']==='POST' && !$checked){
    ...
    $msg='签到成功，获得 '.$points.' 积分，连续 '.$streak.' 天。';
    $checked=true;
    $u=current_user();
}
...
<form class="card" method="post" action="checkin.php">
  <h2>每日签到</h2>
  <p class="muted">基础 10 积分，连续签到每天额外 +1，最多额外 +20。</p>
  <button class="btn" <?=$checked?'disabled':''?>><?=$checked?'今天已签到':'立即签到'?></button>
</form>
<h2>最近签到</h2>
<?php foreach($recent as $r):?><div class="card"><b><?=h($r['check_date'])?></b><span class="muted">+<?=h($r['points'])?>积分 · 连续 <?=h($r['streak'])?> 天</span></div><?php endforeach;?>
<?php shell_end(); ?>
