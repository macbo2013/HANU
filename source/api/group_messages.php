...
if(!$id) json_out(['ok'=>false,'error'=>'群聊不存在']);
...
foreach($rows as $r){
    $out[]=[
        ...
        'username'=>$r['username'],
        'content'=>$r['content'],
        'html'=>render_text_with_links($r['content'],'group_message',(int)$r['id']),
        'time'=>date('m/d H:i',$r['created_at']),
        'mine'=>(int)$r['user_id']===(int)$u['id']
    ];
}
